 ...
 * 	context-security.xml의 쿼리 결과를 변수로 담을 공간을 여기서 생성 후 세션에 사용할 값 저장.
 *  @author User:신숙정
 */
public class EgovSessionMapping extends EgovUsersByUsernameMapping {
	//로거 사용법: 기존에는 System.out.println("디버그"); logger변수를 사용합니다.
	//로거를 사용하는 이유는 배포할때 System.out.~ 이부분을 지우는 것도 일입니다. 이런일을 방지하기 위해서 logger사용
	//보통 new 키워드로 객체를 생성하는데, 이것은 여러번 생성할때, 1번만 생성하는 클래스는 (싱클톤)은 get방식으로 객체를 만듭니다.
	...
	protected EgovUserDetails mapRow(ResultSet rs, int rownum) throws SQLException {
		// EgovUserDetails 맵형 데이터를 전환받는 코드를 개발자가 작성
		logger.debug("디버그메세지");
